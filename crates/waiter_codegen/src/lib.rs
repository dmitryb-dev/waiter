...
pub fn component(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    if let Ok(comp_value) = comp {
        res.extend(unwrap(generate_component_for_struct(comp_value.clone())));
        res.extend(generate_component_provider_impl_struct(comp_value));
        return res;
    }

    let impl_block = syn::parse::<ItemImpl>(item)
...
pub fn provides(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    if let Ok(impl_block_value) = impl_block {
        res.extend(generate_interface_provider_impl(
            provides_attr,
            impl_block_value,
        ));
        return res;
    ...
    let fn_block = syn::parse::<ItemFn>(item)
    ...
    let type_to_wrap = if let Fields::Unnamed(fields) = &wrapper.fields {
        ...
        if field.is_none() {
            return TokenStream::from(
                Error::new(
                    wrapper.span(),
                    "Struct annotated #[wrapper] must have exactly one field",
                )
                .to_compile_error(),
            );
        }
        ...
        return TokenStream::from(
            Error::new(
                wrapper.span(),
                "Only tuple like struct supported for #[wrapper]",
            )
            .to_compile_error(),
        );
    };
